...
namespace Service {
    ...
    {
        float vachat;
        DataTable^ v = this->cad->getRows("SELECT SUM(Prix_HT * Quantite_en_Stock) - SUM((Prix_HT * 0.2) * Quantite_en_Stock) FROM Article");
        return vachat=float::Parse(v->Rows[0]->ItemArray[0]->ToString());
    }
